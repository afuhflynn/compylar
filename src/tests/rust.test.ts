...
describe("Rust adapter", () => {
  it("indexes Cargo source, definitions, tests, and Axum route declarations", async () => {
    ...
    expect(brain.symbols.map((symbol) => symbol.name)).toEqual(
      expect.arrayContaining(["User", "health", "routes", "health_test"]),
    );
    expect(brain.routes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ path: "/health", router: "axum" }),
      ]),
    );
    expect(brain.capabilities).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ adapter: "rust", status: "active" }),
      ]),
    );
  });
});
