...
`RepositoryBrain` is versioned with `brainVersion: 4`. Its top-level collections are packages, files, symbols, routes, dependency edges, diagnostics, ignored projects, reusable memory, learned findings, semantic-index lifecycle metadata, and AI enrichment. A semantic index is complete only after the bundled agent-led codebase index submits a hash-bound, source-cited manifest; older snapshots migrate with semantic status `absent` rather than gaining an unsupported completeness claim.
